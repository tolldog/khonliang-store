...
@pytest.mark.asyncio
async def test_list_returns_empty_list_on_unexpected_shape(make_handler):
    backend, client = _make_backend(make_handler({"error": "boom"}))
    try:
        result = await backend.list()
    finally:
        await client.aclose()
    # The bus's REST surface responds with a list for the list
    # endpoint; if it ever doesn't (error envelope), ensure we
    # don't propagate the dict to the caller as if it were a list.
    assert result == []
